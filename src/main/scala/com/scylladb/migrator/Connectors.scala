...
object Connectors {
  def sourceConnector(sparkConf: SparkConf,
  ...
  def targetConnector(sparkConf: SparkConf,
                      targetSettings: TargetSettings.Scylla) =
    new CassandraConnector(
      CassandraConnectorConf(sparkConf).copy(
        contactInfo = IpBasedContactInfo(
          hosts = Set(
            new InetSocketAddress(targetSettings.host, targetSettings.port)),
