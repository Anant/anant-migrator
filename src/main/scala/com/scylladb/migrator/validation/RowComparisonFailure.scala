...
object RowComparisonFailure {
  ...
  def compareRows(left: CassandraRow,
    ...
    right match {
      case None =>
        Some(RowComparisonFailure(left, right, List(Item.MissingTargetRow)))
      case Some(right) if left.columnValues.size != right.columnValues.size =>
        Some(
          RowComparisonFailure(left,
                               Some(right),
                               List(Item.MismatchedColumnCount)))
      case Some(right)
          if left.metaData.columnNames != right.metaData.columnNames =>
        Some(
          RowComparisonFailure(left,
                               Some(right),
                               List(Item.MismatchedColumnNames)))
      case Some(right) =>
          ...
          for {
            ...
            result = (rightValue, leftValue) match {
              ...
              case (Some(l: Double), Some(r: Double)) =>
                !DoubleMath.fuzzyEquals(l, r, floatingPointTolerance)
              case (Some(l: java.math.BigDecimal),
                    Some(r: java.math.BigDecimal)) =>
                l.subtract(r)
                  .abs()
          ...
          if (!compareTimestamps) Nil
            ...
            for {
              ...
              result <- (leftTtl, rightTtl) match {
                case (Some(l), Some(r))
                    if math.abs(l - r) > ttlToleranceMillis =>
          ...
          if (!compareTimestamps) Nil
            ...
            for {
              ...
              result <- (leftWritetime, rightWritetime) match {
                case (Some(l), Some(r))
                    if math.abs(l - r) > writetimeToleranceMicros =>
                  Some(name -> math.abs(l - r))
                case (Some(l), None)    => Some(name -> l)
                case (None, Some(r))    => Some(name -> r)
                case (Some(l), Some(r)) => None
          ...
          Some(
            RowComparisonFailure(
              ...
              (if (differingFieldValues.nonEmpty)
                ...
                (if (differingTtls.nonEmpty)
                   List(Item.DifferingTtls(differingTtls.toList))
                 else Nil) ++
